...
import { createConnector } from "@ckb-ccc/connector";
...
export default function AvailablePokemon() {
  const [availablePokemon, setAvailablePokemon] = useState<Pokemon[]>([]);
  const [loading, setLoading] = useState(true);
  const [client, setClient] = useState<ccc.Client | null>(null);

  useEffect(() => {
    // Initialize client without requiring wallet connection
    const initClient = async () => {
      const connector = createConnector();
      const newClient = new ccc.Client(connector.client);
      setClient(newClient);
    };
    initClient();
  }, []);
  ...
  const queryPokemonCells = useCallback(async (): Promise<Pokemon[]> => {
    if (!client) return [];
    ...
    try {
      ...
      const pokemonTypeScript = ccc.Script.from({
        ...
        args: createPokemonArgs(CONFIG.ISSUER_LOCK_HASH),
      });
      ...
      for await (const cell of client.findCellsByType(pokemonTypeScript)) {
        // Check if cell is owned by issuer
        if (cell.cellOutput.lock.args === CONFIG.ISSUER_LOCK_ARGS) {
          // Decode Pokemon data inline
          try {
            if (cell.outputData && cell.outputData !== '0x') {
              ...
              if (bytes.length >= 18) {
                ...
                const pokemon = {
                  id: pokemonId,
                  name: pokemonNames[pokemonId] || `pokemon-${pokemonId}`,
                  price,
                  height: 7,
                  weight: 69,
                  base_experience: 64,
  ...
  }, [client, queryPokemonCells]);

  useEffect(() => {
    if (client) {
      loadAvailablePokemon();
    }
  }, [client, loadAvailablePokemon]);

  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-bold mb-4">Available Pokemon</h2>
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <p className="text-gray-600 mb-6">
        These Pokemon are currently available for purchase
      </p>

      {availablePokemon.length === 0 ? (
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🔍</div>
          <h3 className="text-xl font-semibold text-gray-700 mb-2">No Pokemon Available</h3>
          <p className="text-gray-500 mb-4">
            There are no Pokemon NFTs available for purchase at the moment.
          </p>
          <button
